...
export default function NftTag({ index }: { index: number }) {
  const vault = vaults[index];
  return (
    <>
      <div className="absolute -top-6 left-1/2 z-20 mt-0.5 flex w-[161px] -translate-x-1/2 items-center justify-center gap-1 text-center text-sm text-white">
        <img src={vault.nftImage} width={20} alt="nft" />
        <strong>NFT:</strong>
        Chogstars
      </div>
      <svg
